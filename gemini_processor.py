...
"""
    ...
    def select_important_news(self, news_list: List[Dict], target_count: int = 9) -> List[Dict]:
        """
        使用AI从新闻列表中筛选出最重要的新闻
        优先领域（平权）：恒星物理、心理学、神经科学、fMRI
        
        Args:
            news_list: 新闻列表
            target_count: 参考新闻数量（实际可能更多或更少）
        ...
        try:
            ...
            prompt = f"""从以下新闻列表中选择重要的科学新闻。

优先选择标准（以下领域平权，无优先级之分）：
1. 恒星物理、天体物理相关
2. 心理学相关
3. 神经科学相关
4. fMRI、脑成像相关

如果有其他学科的重大科学发现，也应该包含进来。

新闻列表：
{news_text}

要求：
1. 优先选择上述4个领域的新闻（它们之间平权）
2. 如果有其他重大科学发现，也要包含
3. 数量不限，根据新闻质量决定（通常{target_count}条左右，但可以更多或更少）
4. 返回选中的新闻编号，用逗号分隔
5. 只返回编号，如：1,3,5,7,9,12
6. 不要返回其他内容
"""
            ...
            try:
                ...
                # 如果解析失败，取前target_count条
                logger.warning("AI返回格式异常，使用默认选择")
                selected_indices = list(range(min(target_count, len(news_list))))
            ...
            logger.info(f"AI筛选出 {len(selected_news)} 条新闻")
            return selected_news
            
        except Exception as e:
            logger.error(f"AI筛选新闻失败: {e}")
            # 失败时返回前target_count条
...
"""
            ...
            prompt = f"""请批量处理以下 {len(articles)} 篇科学新闻。
